...
class ControlsController extends CoreBase {
	...
	/**
	 * Get the controls on a given page
	 * @returns
	 * @access public
	 */
	getForPage(page) {
	...
	/**
	 * Set active step of a control
	 * @param {string} controlId Id of the control
	 * @param {number} step to set active
	 * @returns {boolean} success
	 * @access public
	 */
	stepSelectCurrent(controlId, step) {
		const control = this.getControl(controlId)
		if (control && typeof control.stepSelectCurrent === 'function') {
			control.stepSelectCurrent(step)
			return true
		}
